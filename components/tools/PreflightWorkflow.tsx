...
export default function PreflightWorkflow() {
  ...
  const commitSnapshot = trpc.decision.logSnapshot.useMutation({
    onSuccess: () => {
      showToast("Mission Logged", "success");
    },
    onError: (error) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      {/* Workflow Stepper */}
      <div className="flex justify-between items-center bg-zinc-900/40 backdrop-blur-xl border border-white/5 p-4 rounded-3xl shadow-2xl">
        {STEPS.map((step, idx) => {
          ...
          return (
            <div key={step.id} className="flex items-center group">
              ...
              {idx < STEPS.length - 1 && (
                <div className="mx-4 md:mx-8 w-8 md:w-16 h-[2px] bg-zinc-800 rounded-full overflow-hidden">
                  <div 
                    className={cn(
            ...
            {aircraft?.map((ac) => (
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="mt-4">{ac.make} {ac.model}</CardTitle>
                </CardHeader>
              </Card>
            ))}
            {loadingAircraft && <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs animate-pulse">Scanning Fleet...</p>}
          </div>
        )}

        {currentStep === "WB" && (
          <div className="bg-zinc-950/40 backdrop-blur-xl border border-white/5 rounded-3xl p-6 shadow-2xl">
          ...
          <div className="space-y-6">
            <Card className="border-2 border-zinc-800 bg-zinc-950/40 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
               ...
               <CardHeader className="text-center">
                  <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ${currentStatus === 'GO' ? 'bg-emerald-500/10' : 'bg-amber-500/10'}`}>
                    {currentStatus === 'GO' ? <ShieldCheck className="w-8 h-8 text-emerald-500" /> : <ShieldAlert className="w-8 h-8 text-amber-500" />}
                  </div>
                  <CardTitle className="text-3xl font-black uppercase italic tracking-tighter">
                    Mission Outlook: {currentStatus}
                  </CardTitle>
                  <CardDescription className="uppercase tracking-[0.2em] font-bold text-[10px]">Consolidated Performance Summary</CardDescription>
               </CardHeader>
            ...
            <Button 
              className="w-full h-16 rounded-3xl bg-blue-600 hover:bg-blue-500 text-xl font-black uppercase italic tracking-tighter transition-all hover:scale-[1.02] active:scale-[0.98]"
              onClick={() => {
            ...
            >
              ...
              {commitSnapshot.isPending ? "Syncing..." : "Finalize & Log Performance"}
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center pt-8">
        <Button 
          ...
          className="h-14 px-8 rounded-2xl border-zinc-800 text-zinc-400 hover:bg-zinc-900"
        >
        ...
        <Button 
          ...
          className="h-14 px-8 rounded-2xl bg-white text-black hover:bg-zinc-200 font-bold"
        >
          {currentStep === "FUEL" ? "Finalize Review" : "Next Step"}
