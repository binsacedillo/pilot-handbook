...
describe('Rate Limiting', () => {
  ...
  describe('getRateLimitKey', () => {
    ...
    it('should return base64 encoded user-agent when no IP header is present', () => {
      const mockReq = {
        headers: new Map(),
      } as unknown as NextRequest;

      const key = getRateLimitKey(mockReq);
      // When no IP is available, it uses base64 encoded user-agent with 'ua:' prefix
      // 'unknown' -> base64 -> 'dW5rbm93bg=='
      expect(key).toBe('ua:dW5rbm93bg==');
    });
  });
