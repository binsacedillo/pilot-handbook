...
describe('flightRouter.getStats', () => {
  ...
  it('aggregates total hours, landings, and currency correctly', async () => {
    flightDb.push(
      { duration: 2, dayLandings: 1, nightLandings: 0, picTime: 2, dualTime: 0, date: new Date() },
      { duration: 3, dayLandings: 2, nightLandings: 0, picTime: 1, dualTime: 2, date: new Date() },
      { duration: 5, dayLandings: 3, nightLandings: 0, picTime: 5, dualTime: 0, date: new Date() },
    );
  ...
  it('handles 0 and missing values without NaN', async () => {
    flightDb.push(
      { duration: 0, dayLandings: 0, nightLandings: 0, picTime: 0, dualTime: 0, date: new Date() },
      { duration: undefined, dayLandings: undefined, nightLandings: undefined, picTime: undefined, dualTime: undefined, date: new Date() },
    );
    const result = await caller.getStats();
