...
### Navigation Standards (ISO 9241-110 Compliance)
We follow the principle of **Interaction Suitability** for navigation:

1. **Contextual Navigation**: Once a user is authenticated, the public "Home" link (Landing Page) is removed from the header.
2. **Logo Redirection**: The top-left logo links to `/dashboard` for authenticated users and `/` for guests.
3. **Automatic Redirection**: Authenticated users visiting the root URL (`/`) are automatically redirected to the dashboard to minimize task friction.

**Desktop:** Horizontal nav with focused application links (Dashboard, Flights, Aircraft).
**Mobile:** Hamburger menu for feature access; Logo for quick dashboard return.
