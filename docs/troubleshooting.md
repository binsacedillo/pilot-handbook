...
### The Fix (Neon Dashboard)

1. Go to **Neon Console** → **Dashboard**
2. Copy your **Connection String**
3. Ensure `DATABASE_URL` is set in your `.env` or Vercel settings.

**Why it works:**
- Neon handles connection pooling automatically via its compute lifecycle.
- You no longer need separate `DIRECT_URL` and `DATABASE_URL` configurations for migrations vs runtime in many cases, although keeping both is safe for Prisma.
- Port **5432** is standard for Neon.

### Verification
1. Check Vercel logs during deployment
2. Look for: `"Migrations executed"` (no errors)
3. Visit deployed app → should load dashboard
