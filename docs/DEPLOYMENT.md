...
Ensure all recent refactors are committed:
...
2. **Critical:** Ensure `DATABASE_URL` includes `?sslmode=verify-full`.

### Step 3: Deploy
1. Click **"Deploy"**.
2. Wait for the build and Prisma client generation to complete (~2 minutes).
3. Get your live URL: `https://your-app.vercel.app`

### Step 4: Apply Prisma Migrations
Run migrations against the production database from a trusted environment after the deployment is ready:

```bash
npx prisma migrate deploy
```

Do not rely on `prisma db push` for production schema changes once the repository has migration history.
...
### ❌ Migration Failed
**Cause:** A migration was not baselined correctly, or existing production rows violate a new enum or constraint.
**Fix:** Inspect the target table data first, correct or baseline as needed, then rerun `npx prisma migrate deploy`.
